...
description: A股实时行情与分时量能分析。获取沪深股票实时价格、涨跌、成交量，分析分时量能分布（早盘/尾盘放量）、主力动向（抢筹/出货信号，需结合大盘环境判断）、涨停/跌停检测、大盘指数（上证/深证/创业板）实时联动、RSI/MACD技术指标。支持持仓管理和盈亏分析。Use when: (1) 查询A股实时行情, (2) 分析主力资金动向, (3) 查看分时成交量分布, (4) 管理股票持仓, (5) 分析持仓盈亏, (6) 盯盘监控。
---

# A股实时行情与分时量能分析

使用东方财富（主）+ 新浪财经（备用）接口获取A股数据，支持实时行情、分时K线、量能分析、涨停/跌停检测、RSI/MACD技术指标、大盘环境联动分析。
